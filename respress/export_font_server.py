...
def health():
    return jsonify({"ok": True, "service": "ResPress export server"})
...
def resolve_legacy_font_path(font_key: str) -> str:
    configured = LEGACY_FONT_PATHS[font_key]
    if os.path.exists(configured):
        return configured
    fallback = os.path.join(BASE_DIR, os.path.basename(configured))
...
def get_export_settings(payload: dict) -> ExportSettings:
    ...
    return ExportSettings(
        ...
        pixel_shape=str(raw.get("pixelShape", "square")).lower(),
...
def set_glyph_from_bitmap(font: TTFont, glyph_name: str, bitmap_payload: dict, pixel_shape: str = "square") -> None:
    ...
    for row in range(rows):
        for col in range(cols):
            ...
            if pixel_shape == "circle":
                ...
                cy = (y_top + y_bottom) / 2
                rx = abs(x1 - x0) * 0.46
                ry = abs(y_top - y_bottom) * 0.46
                k = 0.5522847498307936

                pen.moveTo((round(cx + rx), round(cy)))
                pen.curveTo(
                    (round(cx + rx), round(cy + ry * k)),
                    (round(cx + rx * k), round(cy + ry)),
...
def rebuild_derivative_font(
    ...
    for glyph_name, bitmap_payload in glyph_bitmaps.items():
        if glyph_name not in font.getGlyphOrder():
            continue
        set_glyph_from_bitmap(font, glyph_name, bitmap_payload, pixel_shape=pixel_shape)
...
def build_zip_bytes(
    ...
    derivative_font = rebuild_derivative_font(
        ...
        pixel_shape=settings.pixel_shape,
    )
...
def send_zip_email(email_to: str, export_name: str, zip_name: str, zip_bytes: bytes) -> None:
    ...
    msg["To"] = email_to
    msg.set_content(
        "Here is your custom font export from ResPress.\n\n"
...
def export_font_legacy():
    ...
    font_path = resolve_legacy_font_path(font_key)
...
if __name__ == "__main__":
    host = os.environ.get("EXPORT_FONT_HOST", "0.0.0.0")
    port = int(os.environ.get("PORT") or os.environ.get("EXPORT_FONT_PORT", "8000"))
    debug = os.environ.get("EXPORT_FONT_DEBUG", "0") == "1"
