...
const EXPORT_BACKEND_URL = 'http://127.0.0.1:8000/export-font';
const EXPORT_LEGACY_BACKEND_URL = 'http://127.0.0.1:8000/export-font-legacy';
const EMAIL_BACKEND_URL = 'http://127.0.0.1:8000/email-font';
const EMAIL_LEGACY_BACKEND_URL = 'http://127.0.0.1:8000/email-font-legacy';
...
const translations = {
  en: {
    ...
    exportTitleZip: 'Name custom font export',
    exportTitleEmail: 'Email custom font export',
    exportEmailPlaceholder: 'Email address',
    exportSave: 'Save',
    exportCancel: 'Cancel',
    exportDefaultPng: 'unstable-letters-image',
    exportDefaultZip: 'unstable-letters-font',
    exportProjectLine: 'The Edge of Legibility',
    exportPackaged: 'Includes custom font + export card',
    popupText:
      'Unstable Letters explores the fragile point where language shifts into image and image turns back into language. It looks at legibility as a moving condition instead of a fixed state. By changing resolution, scale, contrast, fonts, and pixel structure, the viewer tests when text stops being readable and begins acting like form, texture, and pattern. The work sits between typography, perception, and interface design, treating letters as unstable visual objects whose meaning can appear, fade, and return.',
    ...
    infoModeMono: 'Mono',
    infoModeColor: 'Color',
    byCredit: 'Unstable Letters by Stefán Daði Karelsson',
  ...
  is: {
    ...
    exportDefaultPng: 'unstable-letters-mynd',
    exportDefaultZip: 'unstable-letters-letur',
    exportProjectLine: 'Mörk læsileikans',
    exportPackaged: 'ZIP inniheldur sérletur og myndkort',
    popupText:
      'Unstable Letters rannsakar augnablikið þar sem tungumál færist yfir í mynd og mynd verður aftur að tungumáli. Verkið lítur á læsileika sem hreyfanlegt ástand fremur en fasta staðreynd. Með því að breyta upplausn, stærð, birtuskilum, letri og pixlabyggingu getur áhorfandinn prófað hvenær texti hættir að vera læsilegur og byrjar að haga sér eins og form, áferð og mynstur. Verkið stendur á mörkum leturfræði, skynjunar og viðmótshönnunar og nálgast stafi sem óstöðug sjónræn form þar sem merking birtist, hverfur og snýr aftur.',
    ...
    infoModeMono: 'Svarthvítt',
    infoModeColor: 'Litur',
    byCredit: 'Unstable Letters eftir Stefán Daða Karelsson',
...
function getTextRenderFont(token, size) {
  if (token.emoji) {
    return `${size}px ${state.emojiFontFamily}`;
  }
...
function drawMixedLine(ctx, line, centerX, baselineY, size, monoColor) {
  const tokens = tokenizeLine(line || ' ');
  ...
  for (const token of tokens) {
    ...
    if (!token.emoji || !state.colorMode) {
...
function drawInfoOverlay(ctx, outW, outH) {
  ...
  const innerPad = Math.round(9 * state.dpr);
  const titleSize = 9.5 * state.dpr;
  const textSize = 8.5 * state.dpr;
  const lineHeight = Math.round(13 * state.dpr);
  const maxPanelW = Math.round(Math.min(outW * 0.18, 260 * state.dpr));
  ...
  const panelW = Math.min(
    maxPanelW,
    Math.max(138 * state.dpr, Math.ceil(measuredW + innerPad * 2))
  );
  const panelH = innerPad * 2 + Math.round(17 * state.dpr) + lineHeight * lines.length;
  const panelX = Math.round(10 * state.dpr);
  const panelY = Math.round(10 * state.dpr);
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ctx.fillText(
      lines[i],
      panelX + innerPad,
      panelY + innerPad + 17 * state.dpr + i * lineHeight,
      panelW - innerPad * 2
    );
  }
...
function drawComposition(ctx, outW, outH, text, options = {}) {
  ...
  const { scaledSize, lines, lineMetrics, paddingY, totalHeight } = getTextLayout(cssWidth, text);
  ...
  for (const line of lines) {
    drawMixedLine(ctx, line || ' ', cssWidth / 2, baselineY, scaledSize, fg);
...
function renderToCanvas(targetCtx, outW, outH, options = {}) {
  ...
  if (sliderValue >= 99.5 || mapped >= 0.995) {
    ...
    if (showInfo && !state.presentationMode) drawInfoOverlay(targetCtx, outW, outH);
    return;
  }
  ...
  targetCtx.drawImage(bufferCanvas, 0, 0, outW, outH);
...
function updateToggleStates() {
  ...
  if (colorBtn) colorBtn.classList.toggle('is-active', state.colorMode);
...
function updateUI() {
  ...
  setText(colorBtn, state.colorMode ? t.colorOn : t.colorOff);
...
function insertEmojiIntoText(emoji) {
  const current = normalizeText(textInput?.value || '');
  const start = textInput?.selectionStart ?? current.length;
  const end = textInput?.selectionEnd ?? current.length;

  const before = current.slice(0, start);
  const after = current.slice(end);
  const nextValue = normalizeText(`${before}${emoji}${after}`);

  if (textInput) {
    textInput.value = nextValue;
    const nextCaret = Math.min(start + emoji.length, nextValue.length);
    textInput.focus();
...
function bindEmojiEvents() {
  emojiOptions.forEach((btn) => {
    btn.addEventListener('click', () => {
      insertEmojiIntoText(btn.dataset.emoji || btn.textContent || '');
    });
  });
}
